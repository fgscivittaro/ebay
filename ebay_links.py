...
def collect_featured_links():
    ...
    Returns: a list of links, each of which corresponds to an eBay featured collection
    """
...
def collect_featured_products(url):
    """
    Takes in the url of a featured collection and returns all the product links within the collection
    ...
    # Iterates through all the URLs found within the HTML code and appends them to product_links
    ...
    # Generates the URL of an xml ajax request responsible for retrieving some but not all of the product links
    editor = url[24:]
    slash = editor.index('/')
    editor = editor[:slash]
    ...
    # Merges the lists and turns them into a set, since there is some overlap between the two
    all_links = list(set(product_links + lxml_links))
    print str(len(all_links)) + " links in all_links"
...
def collect_all_featured_links():
    """
    Iterates through collect_featured_links and returns a combined list of all the featured
    product links within all of the featured collections.
    
    Inputs: none
    
    Returns: A list of every featured url found in all the featured collections on eBay's home page
    """
